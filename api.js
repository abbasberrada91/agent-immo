...
class PropertyAPI {
    ...
    /**
     * Toggle the published status of a property
     * @param {string} reference - Property reference to toggle
     * @param {number} retryCount - Internal retry counter (default: 0)
     * @returns {Object} Result with success status, message, and new isPublished value
     */
    async togglePropertyStatus(reference, retryCount = 0) {
        const token = this.getToken();
        ...
        try {
            ...
            // 3. Toggle isPublished (default to published if field not present)
            const currentlyPublished = biens.properties[index].isPublished !== false;
            biens.properties[index].isPublished = !currentlyPublished;
            biens.properties[index].dateModified = new Date().toISOString();
            ...
            const response = await fetch(
                `https://api.github.com/repos/${this.owner}/${this.repo}/contents/${this.filePath}`,
                {
                    ...
                    body: JSON.stringify({
                        message: `Statut du bien ${reference} mis à jour`,
                        content: content,
                        sha: sha,
                        branch: this.branch
                    })
            ...
            if (!response.ok) {
                ...
                if (this.shouldRetryOperation(errorData, retryCount)) {
                    await this.waitBeforeRetry(retryCount);
                    return this.togglePropertyStatus(reference, retryCount + 1);
                }

                throw new Error(errorData.message || response.statusText);
            }

            const newStatus = biens.properties[index].isPublished;
            return {
                success: true,
                message: `Bien ${reference} ${newStatus ? 'publié' : 'dépublié'} avec succès !`,
