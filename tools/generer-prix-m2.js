...
//
// L'entrée est la réponse de GET /api/admin/dvf/secteur (pige-backend).
//
...
const ORDINAUX = { 1: "1er" };
const ordinal = (n) => ORDINAUX[n] || `${n}e`;

const esc = (s) => String(s == null ? "" : s).replace(/[&<>"]/g, (c) =>
  ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" }[c]));

// Espace insécable fine entre les groupes de chiffres — lisible et jamais coupé en fin de ligne.
const nb = (n) => Number(n).toLocaleString("fr-FR").replace(/ | /g, "&nbsp;");
const eur = (n) => `${nb(n)}&nbsp;€`;
const ppm2 = (n) => `${nb(n)}&nbsp;€/m²`;

function piecesLabel(p) {
  if (p.cinqEtPlus) return "5 pièces et plus";
  return p.pieces === 1 ? "Studio / 1 pièce" : `${p.pieces} pièces`;
}
